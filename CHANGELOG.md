...
## [1.2.2] - 2026-07-15
### Fixed
- **Grouped headers squeezed their columns instead of scrolling.** The row's minimum width summed the top-level column defs, but with grouped headers those are header groups carrying no width of their own, so each group counted as a single default width instead of the total of its leaf columns. The under-counted minimum let the fixed-layout row shrink with the viewport until columns were cut off, and no horizontal scrollbar appeared. Row min-width is now the sum of the visible **leaf** columns (respecting hidden parent groups), so the grid holds its width and overflows into the horizontal scroller. This also restores horizontal drag-scrolling for grouped-header grids, previously there was no overflow to scroll.
- **Grids with `enableVirtualScroll: false` could not be panned horizontally on touch devices.** The grid syncs horizontal scrolling through `<cw-grid-scroller>` rather than a native overflow container, and the touch forwarding only existed on the virtual-scroll host. Non-virtual bodies now handle horizontal drags directly (with axis locking, so vertical swipes still scroll the rows) and set `touch-action: pan-y`.
- **Tabs overflowed the page on narrow screens.** `cw-tabs` laid its header out as a plain flex row, so a tab strip that didn't fit pushed the page wider. The strip now scrolls horizontally instead.
